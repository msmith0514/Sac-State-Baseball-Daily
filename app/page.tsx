...
import { demoDate, liveGames, news, performances, videos } from "@/lib/data/demo";
...
export default function Home() {
  ...
  return <main>
    ...
    {liveGames.length > 0 && <section id="live" className="bg-[#103d2f] text-white">
      <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
          <div>
            <p className="text-xs font-black uppercase tracking-[.22em] text-gold-300">Right now</p>
            <h2 className="athletic-title mt-2 text-4xl sm:text-5xl">Hornets Playing Live</h2>
            <p className="mt-2 max-w-2xl text-sm leading-6 text-white/60">When a tracked player is active, verified game status and available broadcast information appear here.</p>
          </div>
          <p className="rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-bold text-white/55">Demo live status · not real-time sports data</p>
        </div>
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{liveGames.map(game => <LiveGameCard key={`${game.playerSlug}-${game.opponent}`} game={game} />)}</div>
      </div>
    </section>}
